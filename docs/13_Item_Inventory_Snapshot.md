...
- Source: `Campsite - 119h 03m`, modified `2026-08-31 19:40:51 +02:00`, synced into the handbook at `2026-08-31 21:39:47 +02:00`. TMind and Lae'zel moved from Lower City back to camp; Kao and Astarion are unchanged at Baldur's Mouth Basement. **No inventory changes anywhere in the save this pass** — every table below is otherwise identical to the last sync except where noted. All four are now Level 12; Lae'zel respecced into Fighter/Cleric (War Domain) — see [Current Save Snapshot](12_Current_Save_Snapshot.md).
...
No longer carried: **Salty Scimitar(rrr) moved to TMind** this pass — see TMind's section (a genuine downgrade fit-wise; consider moving it to storage instead, since neither current holder can really use it). **Correction on The Joltshooter and Gleamdance Dagger**: the earlier claim that neither was "found anywhere in the save" was wrong — both were sitting there the whole time, just missed. The Joltshooter is with a **vendor NPC** (likely sold, not lost). Gleamdance Dagger is genuinely in camp storage, a short distance from any character rather than exactly on one. **Gold Wyrmling Staff is resolved** — no longer with him, two copies now sit in storage. Also newly present but not rated as build gear: Summon Golem Bell, Spider's Lyre, a secret-elevator ring (name withheld above), and Lute of the Merryweather Bard — see Item Ratings above for why these are quest/cosmetic leftovers rather than active equipment.
