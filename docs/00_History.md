...
- Went further and checked all 195 named items sitting unused in camp storage against bg3.wiki (not just what the four characters carry) — added [[13_Item_Inventory_Snapshot#^storage-notable-gear|Notable Gear Sitting in Storage]] with the Legendary/Very Rare standouts worth actually trying (a legendary dagger and shortsword for Astarion, a legendary sword and a very rare set of armor for Lae'zel, defensive/necromancy picks for TMind and Kao). Also found **True Love's Embrace** sitting in storage — the matching pair for TMind's previously-flagged "dead weight" True Love's Caress ring, which resolves that uncertainty. The other ~175 items checked are cosmetic, outclassed, companion-locked starter gear, or consumables miscategorized as equipment, and were left out of the handbook per the project's practical-not-exhaustive convention.
